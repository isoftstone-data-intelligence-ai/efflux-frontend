      ...
      {messages.length === 0 ? (
        <div className="welcome-message">
          <div className="text-gray-500">
            What can I help you, {userInfo.name} ?
          </div>
        </div>
      ) : (
        messages.map((message: Message, index: number) => {
          return (
            (
              <div
                className={`flex flex-col px-4 shadow-sm whitespace-pre-wrap ${message.role !== 'user'  ? 'bg-accent dark:bg-white/5 border text-accent-foreground dark:text-muted-foreground py-4 rounded-2xl gap-4 w-full' : 'bg-gradient-to-b from-black/5 to-black/10 dark:from-black/30 dark:to-black/50 py-2 rounded-xl gap-2 w-fit'} font-serif  ${message.role == 'user' ?'msgByUse':''}`}
                key={index}
              ...
              >
                {message.content.map((content, id) => {
                  ...
                  if (content.type === 'text') {
                    return (
                      <div><ReactMarkdown>{content.text}</ReactMarkdown></div>
                    )
                  }
                })}
