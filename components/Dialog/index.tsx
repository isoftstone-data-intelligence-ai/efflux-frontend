...
export default class extends Component {
  ...
  render() {
    ...
    var menuList = [
      { name: 'MCP Server', key: 'mcp', icon: '🖥️' },
      // { name: 'MCP Shop', key: 'shop', icon: '🛒' },
    ]
            ...
            <TooltipProvider>
              <Tooltip delayDuration={0}>
                <TooltipTrigger asChild>
                  <Button type='button' variant="ghost" size="icon" id='MCPSettings' className="text-muted-foreground h-6 w-6 rounded-sm">
                    <Image
                      className="flex"
                    ...
                    <div className="w-1/4 bg-white border-r border-gray-300 p-4">
                      <ul className="space-y-2">
                        {menuList.map((item, i) => (
                          <li
                            key={i}
                            className={`flex items-center px-3 py-2 rounded-lg cursor-pointer transition-colors ${selectedMenu === item.key
                              ? 'bg-blue-100 text-blue-600'
                              : 'text-gray-700 hover:bg-gray-100'
                              }`}
                            onClick={() => this.setState({ selectedMenu: item.key })}
                          >
